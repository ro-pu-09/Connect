...
class cartcust extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          actions: [
            FlatButton( child: Text("buy",style: TextStyle(color: Colors.white),),onPressed: ()async {
              dynamic snapshot=await FirebaseFirestore.instance.collection('cartCust').doc(FirebaseAuth.instance.currentUser.email).get();
      ...
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('cartCust').doc(FirebaseAuth.instance.currentUser.email).snapshots() ,
...
writetoordersretailer(String retaileremail, dynamic item,dynamic quant, dynamic status)async {
  ...
  dynamic snapshot= await FirebaseFirestore.instance.collection('custordersto').doc(FirebaseAuth.instance.currentUser.email).get();
  ...
  print(iteml);
  print("------------");
  FirebaseFirestore.instance.collection('custordersto').doc(FirebaseAuth.instance.currentUser.email).set(
    {
      retaileremail:
      {
      "item": iteml,
...
writefromordersretailer(String retaileremail, dynamic item,dynamic quant, dynamic status) async {
  ...
  dynamic snapshot= await FirebaseFirestore.instance.collection('retialordersfrom').doc(retaileremail).get();
  ...
  statusl=List.from(snapshot.data()["status"]);
   ...
   quantl.add(quant);
   statusl.add(status);
  FirebaseFirestore.instance.collection('retialordersfrom').doc(retaileremail).set(
    {
      "item":iteml,
      "quant":quantl,
      "status":statusl,
...
Deleteordercart(String retaileremail, dynamic item,dynamic quant) {
  FirebaseFirestore.instance.collection('cartCust').doc(FirebaseAuth.instance.currentUser.email).update(
    {
